...
final public class SizeAnchor: CompositAnchor {
    ...
    public func equalTo(_ layoutItem: Layoutable, offset: Offset = ( 0, 0)) -> [LayoutConstraint] {
        return [item.width == layoutItem.width + offset.x, item.height == layoutItem.height + offset.y]
    }
...
final public class PositionAnchor: CompositAnchor{
    ...
    public func equalTo(_ anchor: PositionAnchor, offset: Offset = (0,0)) -> [LayoutConstraint] {
        let x = XAxisAnchor(item: item, attribute: attributes.x) == XAxisAnchor(item: anchor.item, attribute: anchor.attributes.x) + offset.x
        let y = YAxisAnchor(item: item, attribute: attributes.y) == YAxisAnchor(item: anchor.item, attribute: anchor.attributes.y) + offset.y
        return [x,y]
    }
    
    @discardableResult
    public func equalTo(_ layoutItem: Layoutable, offset: Offset = (0,0)) -> [LayoutConstraint] {
        return equalTo(PositionAnchor(item: layoutItem, attributes: attributes), offset: offset)
    }
...
final public class XSideAnchor: CompositAnchor {
    ...
    public func equalTo(_ layoutItem: Layoutable, insets: XSideInsets  = (0,0)) -> [LayoutConstraint] {
        return [item.left == layoutItem.left + insets.left, item.right == layoutItem.right - insets.right]
    }
...
final public class YSideAnchor: CompositAnchor {
    ...
    public func equalTo(_ layoutItem: Layoutable, insets: YSideInsets = (0, 0))  -> [LayoutConstraint] {
        return [item.top == layoutItem.top + insets.top, item.bottom == layoutItem.bottom - insets.bottom]
    }
...
final public class EdgeAnchor: CompositAnchor {
    ...
    public func equalTo(_ layoutItem: Layoutable, insets: EdgeInsets = EdgeInsetsZero) -> [LayoutConstraint] {
        return [item.top == layoutItem.top + insets.top, item.left == layoutItem.left + insets.left, item.bottom == layoutItem.bottom - insets.bottom, item.right == layoutItem.right - insets.right]
    }
...
@discardableResult
public func == (lhs: SizeAnchor, rhs: CompositExpression<Layoutable, Offset>) -> [LayoutConstraint] {
