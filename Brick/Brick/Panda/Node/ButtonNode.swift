...
open class ButtonNode: ControlNode {
    ...
    func updateButtonLayout() {
        switch layoutAxis {
        case .horizontal: layoutForHorizontal()
        case .vertical: layoutForVertical()
        }
    }
    ...
    func layoutForHorizontal() {
        ...
        if textFirst {
            let rect = bounds.constraint(lhs: textSize, rhs: imageSize, space: space)
            textNode.frame = rect.0
            imageNode.frame = rect.1
        } else {
            let rect = bounds.constraint(lhs: imageSize, rhs: textSize, space: space)
    ...
    func layoutForVertical() {
        ...
        if textFirst {
            let rects = bounds.constraint(ths: textSize, bhs: imageSize, space: space)
            textNode.frame = rects.0
            imageNode.frame = rects.1
        } else {
            let rects = bounds.constraint(ths: imageSize, bhs: textSize, space: space)
